...
function FooterNavSection({ title, links }: NavigationSection) {
  return (
    <div className="flex min-w-[120px] flex-1 flex-col items-start justify-start gap-3">
      <h3 className="self-stretch font-medium font-sans text-[rgba(73,66,61,0.50)] text-sm leading-5">
        {title}
      </h3>
      ...
      <div className="absolute inset-0 h-full w-full overflow-hidden">
        <div className="relative h-full w-full">
          {Array.from({ length: 400 }).map((_, i) => (
            <div
              className="-top-[120px] -rotate-45 absolute h-16 w-[300px] origin-top-left border border-[rgba(3,7,18,0.08)]"
              // biome-ignore lint/suspicious/noArrayIndexKey: Pattern generation requires index
              key={i}
              style={{
                left: `${i * STRIPE_HORIZONTAL_OFFSET - STRIPE_VERTICAL_OFFSET}px`,
              }}
            />
          ))}
        </div>
...
function Footer() {
  return (
    <footer className="flex w-full flex-col items-start justify-start pt-10">
      {/* Main Footer Content */}
      <div className="flex h-auto flex-col items-stretch justify-between self-stretch pb-8 md:flex-row">
        ...
        <div className="flex h-auto flex-col items-start justify-start gap-8 p-4 md:p-8">
          <div className="flex items-center justify-start gap-3 self-stretch">
            <h2 className="text-center font-charm text-[#0D4A85] text-xl leading-4">
              Coral Infantil de Setúbal
            </h2>
          </div>

          <address className="font-medium font-sans text-[rgba(73,66,61,0.90)] text-sm not-italic leading-[18px]">
        ...
        {/* Navigation Sections */}
        <nav className="flex flex-col flex-wrap items-start justify-start gap-6 self-stretch p-4 sm:flex-row sm:justify-between md:gap-8 md:p-8">
